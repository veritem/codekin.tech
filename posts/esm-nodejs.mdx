...
### Esm vs Babel
  ...
  1. Esm is browser compatible but  Babel's transipiration is not because it uses nodejs module system which is commonJS and commonJs doesn't run in browser.
  3. Esm is statically parsed, Babel transipiration is not.
  Esmodules is statically parsed while Babel's transpired is is parsed and excuted at the same time.
  4. Esm is async and Supports top level await, Babel transipiration is not and doesn't.
  5. ESM is native, Babel is a transipiration.
